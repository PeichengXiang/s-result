...
- 总榜每模型类型使用同一权重的全任务等权平均最高值。与评测 Web 一致，优先取同权重有成功记录的完整测评轮次中平均最高的一次；若没有此类轮次（包括完整批次全部为 0 分），则各任务取最佳完整单次，避免旧全零批次遮蔽后续重测成绩。测评轮次沿用 Web 批次分组，不能用单任务 run ID 拆分完整批次。仅覆盖部分任务或总分为 0 的模型不参与全任务总榜；有效的单任务 0 分和原始记录仍保留。
