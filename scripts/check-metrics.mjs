...
import {data,ranking,selectedCells,formatResultUpdateTime,egoVlaLeaderboard,egoVlaTaskGroups,egoVlaSetting} from '../work/metrics.mjs';
...
const e=data.benchmarks.find(b=>b.id==='egovla');const h=e.models.find(m=>m.label==='H-RDT');const drawer=e.tasks.find(t=>t.source==='Humanoid-Stack-Can-Into-Drawer-v0');const record=e.records.find(p=>p.modelId===h.id&&p.taskId===drawer.id&&p.epoch===80000);assert.ok(Math.abs(record.rate-7/66)<1e-12);const egoRows=selectedCells(e);assert.ok(egoRows.every(row=>row.seen!==null),'EgoVLA selected rows must preserve Seen SR');egoRows.forEach(row=>{const cells=row.cells.filter(Boolean);assert.ok(Math.abs(row.seen-cells.reduce((sum,cell)=>sum+cell.seen,0)/row.coverage)<1e-12,'EgoVLA Seen SR must be the selected-task average')});const actEgo=egoRows.find(x=>x.model.policy==='act');assert.ok(actEgo&&actEgo.coverage===12,'Merged ACT EgoVLA result must remain a single model with all twelve tasks');assert.ok(ranking(e).some(x=>x.model.policy==='act'),'Merged ACT EgoVLA result must enter the formal ranking once complete');const groups=egoVlaTaskGroups(e);assert.equal(groups.short.length,7);assert.equal(groups.long.length,5);for(const horizon of ['short','long'])for(const split of ['seen','unseen']){const rows=egoVlaLeaderboard(e,horizon,split);assert.ok(rows.every(row=>row.coverage===groups[horizon].length));assert.ok(new Set(rows.map(row=>row.setting.key)).size===rows.length)}
const setting=(name,policy='spark_0')=>egoVlaSetting({name,policy,label:name,id:name});assert.equal(setting('egovla_spark_visual_pretrain').key,'spark_visual_pretrain');assert.equal(setting('egovla_spark_tactile100h_pretrain').key,'spark_tactile_100h_pretrain');assert.equal(setting('spark0-0914-Inspire12-EgoVLA').key,'spark_inspire');assert.equal(setting('egovla_spark_tactile100h_pretrain_qpos0_once_free').key,'spark_no_pretrain');assert.equal(setting('Spark0-0908-query-plus-visual').key,'spark_no_pretrain');
console.log('H-RDT restored Unseen score, Seen SR aggregation, EgoVLA short/long setting selection, ACT grouping, weight isolation and publication allowlist passed');
